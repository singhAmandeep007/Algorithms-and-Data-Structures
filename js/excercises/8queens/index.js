...
function EightQueens1(strArr) {
  let formatedArr = strArr.map((pos) => {
    return pos.replace(/[()]/g, "");
  });

  function checkIsAttacking(currRow, currCol, currIndex) {
    let flag = true;
    // dont run for first
    for (let j = currIndex + 1; j < formatedArr.length; j++) {
      let [row, col] = formatedArr[j].split(",");
      row = +row;
      col = +col;
...
module.exports = [EightQueens, EightQueens1];
