import { chunk1, chunk2, chunk3, chunk4 } from "./index";

describe("Chunk", () => {
  [chunk1, chunk2, chunk3, chunk4].forEach((chunk) => {
    ...
    test("chunk divides an array of 1 element with chunk size 5", () => {
      const arr = [10];
      const chunked = chunk(arr, 0);

      expect(chunked).toEqual(arr);
    });
  });
});
